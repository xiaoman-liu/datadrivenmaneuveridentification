...
def build_lstm_model(features,weights = np.array(
    ...
    model = Sequential()
    model.add(Masking(mask_value=0, input_shape=features.shape[1:]))
    model.add(LSTM(1024, return_sequences=True, input_shape=features.shape[1:]))
...
def build_fcn_model(features,weights = np.array(
    ...
    model = Sequential()
    # model.add(Masking(mask_value=0, input_shape=features.shape[1:]))
    model.add(Conv1D(filters=64, kernel_size=3, padding='same', input_shape=features.shape[1:])) #1
    model.add(BatchNormalization())
    model.add(Activation(activation= "relu"))

    model.add(Conv1D(filters=128, kernel_size=3, padding='same'))#2
    model.add(BatchNormalization())
    model.add(Activation("relu"))

    model.add(Conv1D(filters=128, kernel_size=3, padding='same'))#3
    model.add(BatchNormalization())
    model.add(Activation("relu"))

    model.add(Conv1D(filters=128, kernel_size=3, padding='same'))#4
    model.add(BatchNormalization())
    model.add(Activation("relu"))

    model.add(Conv1D(filters=128, kernel_size=3, padding='same'))#5
    model.add(BatchNormalization())
    model.add(Activation("relu"))

    model.add(Conv1D(filters=128, kernel_size=5, padding='same'))#6
    model.add(BatchNormalization())
    model.add(Activation("relu"))

    model.add(Conv1D(filters=128, kernel_size=5, padding='same'))#7
    model.add(BatchNormalization())
    model.add(Activation("relu"))

    model.add(Conv1D(filters=128, kernel_size=5, padding='same'))#8
    model.add(BatchNormalization())
    model.add(Activation("relu"))

    model.add(Conv1D(filters=64, kernel_size=7, padding='same'))#9
    model.add(BatchNormalization())
    model.add(Activation("relu"))
...
def build_blstm_resnet_model(features, weights  ,class_number = 4):
    ### features.shape = (579,777,17)
    ip = Input(features.shape[1:])

    x = Masking(mask_value=0, input_shape=features.shape[1:])(ip)
    x = Bidirectional(LSTM(256, return_sequences=True, input_shape=features.shape[1:]))(x)
    x = Bidirectional(LSTM(128, return_sequences=True, input_shape=features.shape[1:]))(x)
    x = Bidirectional(LSTM(64, return_sequences=True, input_shape=features.shape[1:]))(x)
    ...
    # Resnet
    conv_x = Conv1D(filters=n_feature_maps * 2, kernel_size=3, padding='same')(input_layer)
    ...
    conv_z = Conv1D(filters=n_feature_maps * 2, kernel_size=7, padding='same')(conv_y)
    conv_z = BatchNormalization()(conv_z)
    conv_z = Activation('relu')(conv_z)

    # expand channels for the sum
    shortcut_y = Conv1D(filters=128, kernel_size=1, padding='same')(input_layer)
    shortcut_y = BatchNormalization()(shortcut_y)

    output_block_1 = add([shortcut_y, conv_z])
    output_block_1 = Activation('relu')(output_block_1)
    output_block_1 = Dense(64)(output_block_1)

     #y.shape = (?,128)

    x = Concatenate(axis = 2)([x, output_block_1])
...
def build_resnet_model(features,weights = np.array(
    ...
    # expand channels for the sum
    shortcut_y = Conv1D(filters=n_feature_maps * 2, kernel_size=1, padding='same')(input_layer)
    shortcut_y = BatchNormalization()(shortcut_y)
    ...
    # # BLOCK 2
    # conv_x = Conv1D(filters=n_feature_maps * 2, kernel_size=7, padding='same')(output_block_1)
    # conv_x = BatchNormalization()(conv_x)
    # conv_x = Activation('relu')(conv_x)
    #
    # conv_y = Conv1D(filters=n_feature_maps * 4, kernel_size=9, padding='same')(conv_x)
    # conv_y = BatchNormalization()(conv_y)
    # conv_y = Activation('relu')(conv_y)
    #
    # conv_z = Conv1D(filters=n_feature_maps * 2, kernel_size=9, padding='same')(conv_y)
    # conv_z = BatchNormalization()(conv_z)
    #
    # # expand channels for the sum
    ...
    #
    # # # BLOCK 3
    # conv_x = Conv1D(filters=n_feature_maps * 2, kernel_size=7, padding='same')(output_block_2)
    # conv_x = BatchNormalization()(conv_x)
    # conv_x = Activation('relu')(conv_x)
    #
    # conv_y = Conv1D(filters=n_feature_maps * 4, kernel_size=7, padding='same')(conv_x)
    # conv_y = BatchNormalization()(conv_y)
    # conv_y = Activation('relu')(conv_y)
    #
    # conv_z = Conv1D(filters=n_feature_maps * 2, kernel_size=7, padding='same')(conv_y)
    ...
    output_layer = TimeDistributed(Dense(class_number, activation='softmax'))(output_block_1)
