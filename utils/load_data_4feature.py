import numpy as np
from TypeMapping import lateral_id_mapping,longitudinal_id_mapping,laneID_mapping,section_roadid
...
def load_scenario_data():
    ...
    features = np.zeros((sample_number, max_timestep, 2+1+28+16), dtype=np.float32)
    ...
    for i, scenario_data in enumerate(scenarios_data):
        ...
        for key in scenario_data:
            ...
            signal_value = scenario_data[key]['signals'].values[:, list(range(3,5))]
            ...
            features[index, :signal_value.shape[0], signal_value.shape[1] :signal_value.shape[1] + 1] = road_is_section# shape, (num_sample, max_timestep, 12)
            features[index, :signal_value.shape[0], signal_value.shape[1] + 1 :signal_value.shape[1] + 29] = onehot_roadid# shape, (num_sample, max_timestep, 12)
            features[index, :signal_value.shape[0], signal_value.shape[1] + 29:signal_value.shape[1] + 45] = onehot_laneid
