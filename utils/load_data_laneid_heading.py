...
def load_scenario_data():
    ...
    features    = np.zeros((sample_number, max_timestep, 1+1), dtype=np.float32)
    ...
    for i, scenario_data in enumerate(scenarios_data):
        ...
        for key in scenario_data:
            ...
            #     position = first_laneid.index(laneid[id])
            #     if_changelane[id] = position
            # if len(first_laneid) > 1:
            #     if_holdlane-=1
            # if_changelane = k_utils.to_categorical(if_changelane, num_classes=7)
            ...
            # laneid
            signal_value = scenario_data[key]['signals'].values[:, 12]
            ...
            features[index, :signal_value.shape[0], :2] = if_changelane
            # features[index, :signal_value.shape[0], 2:3] = signal_value
