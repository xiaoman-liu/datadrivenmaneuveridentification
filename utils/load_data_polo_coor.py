...
def load_scenario_data():
    ...
    features    = np.zeros((sample_number, max_timestep, 6 + 2+2 +3 +2+2+ 5+1), dtype=np.float32)
    ...
    for i, scenario_data in enumerate(scenarios_data):
        ...
        for key in scenario_data:
            ...
            # x,y
            postion_x = np.array(scenario_data[key]['signals'].values[:, 1],dtype = np.float32)
            position_y = np.array(scenario_data[key]['signals'].values[:, 2],dtype = np.float32)
            angel = np.arctan(position_y / postion_x) * 180 / math.pi/360
            angel = angel.reshape(-1,1)
            ...
            lane_direction = np.zeros_like(laneid)
            ...
            features[index, :signal_value.shape[0], total_signal_shape + 14:total_signal_shape + 16] = lane_direction
            features[index, :signal_value.shape[0], total_signal_shape + 14:total_signal_shape + 15] = angel
