...
def load_scenario_data():
    ...
    features    = np.zeros((sample_number, max_timestep, 1+ 1 + 28 + 4), dtype=np.float32)
    ...
    for i, scenario_data in enumerate(scenarios_data):
        ...
        for key in scenario_data:
            ...
            # numerical feature
            signal_value       = scenario_data[key]['signals'].values[:, [0]] # time , scoordinate, roadyaw,shape = ()
            ...
            # add features
            total_signal_shape =  signal_value.shape[1] #signal_value_minus.shape[1] +
            ...
            features[index, :signal_value.shape[0], total_signal_shape + 29:total_signal_shape + 31] = if_holdlane
            features[index, :signal_value.shape[0], total_signal_shape + 31:total_signal_shape + 33] = if_holdlane
