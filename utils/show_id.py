...
import numpy as np
from TypeMapping import lateral_id_mapping,longitudinal_id_mapping,sample_type_set,laneID_mapping
...
def load_scenario_data():
    ...
    features = np.zeros((sample_number, max_timestep, 11+11+16+5), dtype=np.float32)
    ...
    for i, scenario_data in enumerate(scenarios_data):
        ...
        for key in scenario_data:
            ...
            signal_value = scenario_data[key]['signals'].values[:, list(range(0,7)) + list(range(9,13))]
            ...
            # lane_type = scenario_data[key]['signals'].values[:, 9]
            ...
            features[index, :signal_value.shape[0], signal_value.shape[1] + 27:signal_value.shape[1] + 32] = onehot_type
